...
func TestTier2Call(t *testing.T) {
	...
	jsStoreSourceKeyInit := hex.EncodeToString([]byte("js_store_source_key"))
	// jsMapFromStoreDeltaInit := hex.EncodeToString([]byte("js_test_map_from_store_delta"))
	// jsStoreFromStoreKeyInit := hex.EncodeToString([]byte("js_store_source_key"))
	// jsStoreFromStoreDeltaInit := hex.EncodeToString([]byte("js_test_store_from_store_delta"))
	...
	cases := []struct {
...

		{
			...
			stateBundleSize: 10,
			manifestPath:    "./testdata/js_substreams/substreams_js.spkg",
		...
		{
			name:            "js_store_source_key",
			startBlock:      10,
			stage:           0,
			moduleName:      "js_store_source_key",
			stateBundleSize: 10,
			manifestPath:    "./testdata/js_substreams/substreams_js.spkg",

			expectRemainingFiles: []string{
				jsStoreSourceKeyInit + "/states/0000000010-0000000020.kv",
			},
		},
